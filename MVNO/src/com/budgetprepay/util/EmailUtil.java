...
public class EmailUtil {
	...
	public EmailUtil() { 
		...
		properties.put("mail.host", ResourceUtil.get("email.server"));
	}
	...
	public void send(final String recipient) throws MessagingException, InterruptedException {
	    ...
	    message.setFrom(new InternetAddress(ResourceUtil.get("email.account")));
	    message.setRecipients(Message.RecipientType.TO, InternetAddress.parse(recipient));
	    Transport transport = session.getTransport();
	    for (int j = 0 ; j < 5 ; j++) {
	       transport.connect(ResourceUtil.get("email.account"), ResourceUtil.get("email.password"));
	...
	public void sendXML(final String recipient, final String xml) throws MessagingException, InterruptedException {
		if (recipient == null) return;
		session = Session.getInstance(properties);
	    MimeMessage message = new MimeMessage(session);
		message.setContent(xml, "application/xml");
	    message.setSubject(subject);
	    message.setFrom(new InternetAddress(ResourceUtil.get("email.account")));
	    message.setRecipients(Message.RecipientType.TO, InternetAddress.parse(recipient));
	    Transport transport = session.getTransport();
