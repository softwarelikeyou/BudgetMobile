...
public class AsyncListWindow extends GenericAbstractListWindow implements AfterCompose  {
	...
	protected List<PlintronAsyncResponse> getFullList() {	
		...
		try {
			if (filter == null)
		        list = PlintronAsyncResponseFacade.findAll();
			else
				list = PlintronAsyncResponseFacade.findAll(filters);
		}
		catch (Exception e) {
	...
	public void onSelect$asyncListbox(Event event) {
		PlintronAsyncResponse request = (PlintronAsyncResponse) WebUtil.getRowBindingData(event);
		Events.postEvent(new Event("onAsyncSelected", null, request));
	}
